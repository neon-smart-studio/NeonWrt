...
define prepare_rootfs
	...
	#
	# Basic root filesystem layout
	#
	@mkdir -p $(1)/etc/rc.d

	#
	# NeonWrt/systemd runtime hierarchy
	#
	# systemd PID 1 mounts a tmpfs on /run during very early boot.
	# /run therefore MUST physically exist in the final rootfs image
	# before the kernel executes /sbin/init.
	#
	@mkdir -p $(1)/run
	@mkdir -p $(1)/run/lock
	@chmod 0755 $(1)/run
	@chmod 0755 $(1)/run/lock

	#
	# OpenWrt historically expects /var/lock to exist.
	# NeonWrt/systemd uses:
	#
	#   /var/run  -> /run
	#   /var/lock -> /run/lock
	#
	# If /var/lock is already a symlink, do not replace it.
	#
	@if [ ! -L "$(1)/var/lock" ]; then \
		mkdir -p "$(1)/var/lock"; \
	fi
	...
	@( \
		...
		for script in $$IPKG_POSTINST_PATH; do \
			...
			[ -n "$(CONFIG_USE_APK)" ] && \
				$(STAGING_DIR_HOST)/bin/tar --delete \
					-f ./lib/apk/db/scripts.tar \
					$$(basename $$script); \
		done; \
		[ -n "$(CONFIG_USE_APK)" ] && \
			$(STAGING_DIR_HOST)/bin/gzip -f -9n -S ".gz" \
		...
		if [ -z "$(CONFIG_USE_APK)" ]; then \
			...
			$(if $(SOURCE_DATE_EPOCH), \
				sed -i \
					"s/Installed-Time: .*/Installed-Time: $(SOURCE_DATE_EPOCH)/" \
	...
	#
	# Final systemd filesystem sanity.
	#
	# Keep this AFTER the cleanup above so the image passed to mkfs
	# is guaranteed to contain the early /run mount point.
	#
	@mkdir -p $(1)/run
	@mkdir -p $(1)/run/lock
	@chmod 0755 $(1)/run
	...
	$(if $(SOURCE_DATE_EPOCH), \
		find $(1)/ -mindepth 1 \
			-execdir touch -hcd "@$(SOURCE_DATE_EPOCH)" "{}" +)
endef